...
class WC_HipayEnterprise_Notification
{
                ...
                case TransactionStatus::CAPTURE_REQUESTED: //117
                    ...
                    if ($this->transaction->getCapturedAmount() < $this->transaction->getAuthorizedAmount()) {
                        $this->updateOrderStatus(
                            ...
                            0,
                            1,
                            __("Payment partially captured, amount:." ." " . $this->transaction->getCapturedAmount() , 'hipayenterprise') . " " .  $this->transaction->getTransactionReference(), 0);
                    } else {
                        $this->updateOrderStatus(
                            $orderState,
                            1,
                            1,
                            __("Payment successful for transaction", 'hipayenterprise') . " " . $this->transaction->getTransactionReference());
    ...
    public function updateOrderStatus($status,
                                      ...
                                      $message) {
        global $wpdb;
        $wpdb->update($this->plugin->plugin_table, array(
            'captured' => $captured,
            'reference' => $this->transaction->getTransactionReference(),
            'status' => $status,
            'operation' =>  $this->transaction->getMessage() ,
            'processed' => $processed,
            'processed_date' => date('Y-m-d H:i:s')),
            array($this->transaction->getOrder()->getId(), 'processed' => 0 ));
