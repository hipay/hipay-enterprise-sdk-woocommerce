 ...
 * @since 1.0.0
 */
class WC_HipayEnterprise_LocalPayments_Multibanco extends WC_HipayEnterprise {
	...
	public function __construct() {
		...
		$this->method_title         = __('Multibanco','hipayenterprise');
		...
		try {
			...
			$redirectUrl = $transaction->getForwardUrl();
				...
				if ($redirectUrl != ""){
			    	...
			    	if ($this->method_details['woocommerce_hipayenterprise_methods_log_info'])
						$wpdb->insert( $this->plugin_table_logs, array( 'log_desc' => __('Entity:Error generating payment url.','hipayenterprise'), 'order_id' => $order_id, 'type' => 'ERROR' ) );
