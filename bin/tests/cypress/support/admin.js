...
Cypress.Commands.add('switchWooCurrency', (currency) => {
  cy.get('#toplevel_page_woocommerce > .wp-submenu > :nth-child(5) > a').click({
    force: true
  });
...
Cypress.Commands.add('goToAdminHipayConfig', () => {
  cy.get(
    '[data-gateway_id="hipayenterprise_credit_card"] > .name > .wc-payment-gateway-method-title'
  ).click({ force: true });
});
...
Cypress.Commands.add('activatePaymentMethods', (method) => {
  cy.get(
    '[data-gateway_id="hipayenterprise_' +
      method +
      '"] > .status > .wc-payment-gateway-method-toggle-enabled > .woocommerce-input-toggle'
  ).then(($btn) => {
...
Cypress.Commands.add('goToOrderTab', () => {
  cy.get(
    '#toplevel_page_woocommerce > .wp-submenu > li.wp-first-item > .wp-first-item'
  ).click({ force: true });
});
...
Cypress.Commands.add('resetCCConfigForm', () => {
  cy.get('#woocommerce_hipayenterprise_methods_visa_minAmount').clear({
    force: true
  });
  cy.get('#woocommerce_hipayenterprise_methods_mastercard_minAmount').clear({
    force: true
  });
  cy.get('#woocommerce_hipayenterprise_methods_cb_minAmount').clear({
    force: true
  });
  cy.get('#woocommerce_hipayenterprise_methods_maestro_minAmount').clear({
    force: true
  });
  cy.get(
    '#woocommerce_hipayenterprise_methods_american-express_minAmount'
  ).clear({ force: true });
  cy.get('#woocommerce_hipayenterprise_methods_bcmc_minAmount').clear({
    force: true
  });
  cy.get('#woocommerce_hipayenterprise_methods_visa_activated').check({
    force: true
  });
  cy.get('#woocommerce_hipayenterprise_methods_mastercard_activated').check({
    force: true
  });
  cy.get('#woocommerce_hipayenterprise_methods_cb_activated').check({
    force: true
  });
  cy.get('#woocommerce_hipayenterprise_methods_maestro_activated').check({
    force: true
  });
  cy.get(
    '#woocommerce_hipayenterprise_methods_american-express_activated'
  ).check({ force: true });
  cy.get('#woocommerce_hipayenterprise_methods_bcmc_activated').check({
    force: true
  });
});
...
Cypress.Commands.add('activateOneClick', () => {
  cy.logToAdmin();
  cy.visit(
    '/wp-admin/admin.php?page=wc-settings&tab=checkout&section=hipayenterprise_credit_card'
  );
  cy.get('#methods-tab').click();
...
Cypress.Commands.add('mapCategories', () => {
  cy.get(
    '#toplevel_page_hipay-settings.wp-has-submenu a.wp-first-item'
  ).contains('Mapping category');
  cy.get('#toplevel_page_hipay-settings.wp-has-submenu a.wp-first-item').click({
    force: true
  });
  cy.get(
    'table.table-striped tbody tr:nth-child(1) select.form-control'
  ).select('Home & Gardening');
  cy.get(
    'table.table-striped tbody tr:nth-child(2) select.form-control'
  ).select('Home appliances');
  cy.get(
    'table.table-striped tbody tr:nth-child(3) select.form-control'
  ).select('Home appliances');
  cy.get(
    'table.table-striped tbody tr:nth-child(4) select.form-control'
  ).select('Home appliances');
  cy.get(
    'table.table-striped tbody tr:nth-child(5) select.form-control'
  ).select('Home appliances');
  cy.get(
    'table.table-striped tbody tr:nth-child(6) select.form-control'
  ).select('Home & Gardening');
...
Cypress.Commands.add('mapCarriers', () => {
  cy.get('#toplevel_page_hipay-settings.wp-has-submenu a ')
    .contains('Mapping delivery method')
    .click({ force: true });
  cy.get(
    'table.table-striped tbody tr:nth-child(1) input[name="mapping_order_preparation_flat_rate"]'
  ).clear();
  cy.get(
    'table.table-striped tbody tr:nth-child(1) input[name="mapping_delivery_estimated_flat_rate"]'
  ).clear();
  cy.get('.button-primary').click();
  cy.get('#message').contains('Your settings have been saved.');
  cy.get(
  ...
  cy.get(
    'table.table-striped tbody tr:nth-child(1) select[name="mapping_mode_flat_rate"]'
  ).select('store');
  cy.get(
    'table.table-striped tbody tr:nth-child(1) select[name="mapping_shipping_flat_rate"]'
  ).select('standard');
  cy.get('.button-primary').click();
  cy.get('#message').contains('Your settings have been saved.');
});
...
Cypress.Commands.add('getAllRequests', () => {
  ...
  cy.get('select[name="log_file"] option')
    .contains('hipayenterprise_credit_card-request')
    .then(($option) => {
      ...
      cy.get('#log-viewer pre')
        .invoke('text')
        .then((text) => {
          let rawLogArray = text.split(
            /^20[0-9]{2}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\+[0-9]{2}:[0-9]{2} DEBUG /gm
          );
          let logArray = [];
